...
public class HourlySlotsController {
    ...
    public ResponseEntity<List<HourlySlotModel>> getHourlySlotsByDate(
        ...
        return ResponseEntity.ok(hourlySlotService.getHourlySlotsByDate(fieldId, pitchId, slotDate));
    }

    @PutMapping("/{id}/status")
    public ResponseEntity<HourlySlotModel> updateSlotStatus(
            @PathVariable int id,
            @RequestBody SlotStatus status) {
        return ResponseEntity.ok(hourlySlotService.updateHourlySlotStatus(id, status));
    }
}
